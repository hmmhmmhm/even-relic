...
- Buffer the assistant's authoritative Realtime text and expose one Unicode
  grapheme every 20 ms. New upstream deltas replace the pending target rather
...
The 20 ms clock is a presentation clock, not an SDK call queue. Native
...
- Fake-timer tests prove one grapheme per 20 ms, including composed emoji.
